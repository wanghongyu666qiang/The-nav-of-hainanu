...
Graph getWalkGraph(const Graph& g) {
    // 琛屼汉鍥撅細鍙繚鐣?roadType == 0 鐨勮竟
    extern Graph buildFilteredGraph(const Graph& g, int allowedRoadType, bool filterByType);
    return buildFilteredGraph(g, 0, true);
}
...
Graph getDriveGraph(const Graph& g) {
    // 椹捐溅鍥撅細鍙繚鐣?roadType == 1 鐨勮竟
    extern Graph buildFilteredGraph(const Graph& g, int allowedRoadType, bool filterByType);
    return buildFilteredGraph(g, 1, true);
}

// 杈呭姪锛氭牴鎹厑璁哥殑閬撹矾绫诲瀷鏋勫缓涓€涓€傜敤浜庢渶鐭矾绠楁硶鐨勯偦鎺ョ煩闃靛浘
Graph buildFilteredGraph(const Graph& g, int allowedRoadType, bool filterByType) {
    Graph res = g;
    int n = (int)res.scenes.size();
    for (int i = 0; i < n; ++i) {
        vector<Edge> matrixEdges(n);
        for (int j = 0; j < n; ++j) matrixEdges[j] = {res.scenes[j].id, INT_MAX, allowedRoadType};
        matrixEdges[i].weight = 0;
        for (const Edge& e : g.scenes[i].edges) {
            if (!filterByType || e.roadType == allowedRoadType) {
                int targetIndex = -1;
